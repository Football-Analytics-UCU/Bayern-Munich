...
def create_pass_heatmap(data, team, n_clusters=40):
    ...
    t_kmean = KMeans(n_clusters).fit(pass_data[['location_x', 'location_y', 'pass_end_location_x', 'pass_end_location_y']])
    ...
    for i in range(len(pass_clusters['location_x'])):
        try:
            if pd.isnull(pass_clusters['pass_outcome'][i]):
                plt.plot((pass_clusters['location_x'][i], pass_clusters['pass_end_location_x'][i]), (pass_clusters['location_y'][i], pass_clusters['pass_end_location_y'][i]), color='green')
                plt.scatter(pass_clusters['location_x'][i], pass_clusters['location_y'][i], color='green')
            if pd.notnull(pass_clusters['pass_outcome'][i]):
                plt.plot((pass_clusters['location_x'][i], pass_clusters['pass_end_location_x'][i]), (pass_clusters['location_y'][i], pass_clusters['pass_end_location_y'][i]), color='red')
...
def plot_hulls(team_dataset, lineups, team, main_color):
    ...
    for idx, player in enumerate(unique_players):
        ...
        pos = lineups[lineups['player_name'] == player]['positions']
